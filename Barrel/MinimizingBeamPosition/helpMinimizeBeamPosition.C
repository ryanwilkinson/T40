...
I have attempted to get a spectrum of Phi angle vs Hit Z position from my data.
I went to the part of this code where the Z coordinates were calculated for the minimised data (in the GetChiSquare function). There, I replaced the angles originally used to calculate the hit z position with the angles specificed in the input file. The rest of the code is the same as previously.
I am unsure as to whether these changes are correct, or if they are whether they should eb calculated at this part of the code. All I know is that by running this code with the "elastic_angles.txt" file I get some strange looking results.
...
//Global variables
  ...
  vector <TVector3> gHitOnStripData;
  vector <double> gHitOnStripZ;
  vector <double> gHitOnStripZData;
...
// MAIN
void MinimizeBeamPosition2(double Angle=67.5584, TString data="./scripts/MinimizingBeamPosition/elastic_angles.txt", double X=0/*1.62025*//*-1.36*/, double Y=0/*5.12941*//*+0.65*/, double Z=0/*2.30263*//*6.00*/){

  gTargetPos.SetXYZ(X,Y,Z);
...
// perform the numerical minimisation
  ...
  /*vector<double> mockAngles, mockHyperStrip;
  mockAngles.push_back(70.4167); mockHyperStrip.push_back(1);
  mockAngles.push_back(70.9142); mockHyperStrip.push_back(2);
  mockAngles.push_back(71.4157); mockHyperStrip.push_back(3);
  mockAngles.push_back(71.833);  mockHyperStrip.push_back(4);
  mockAngles.push_back(72.7241); mockHyperStrip.push_back(5);
  mockAngles.push_back(73.5326); mockHyperStrip.push_back(6);
  mockAngles.push_back(74.1921); mockHyperStrip.push_back(7);
  mockAngles.push_back(74.5709); mockHyperStrip.push_back(8);
  mockAngles.push_back(75.3516); mockHyperStrip.push_back(9);
  mockAngles.push_back(76.0402); mockHyperStrip.push_back(10);
  mockAngles.push_back(76.3694); mockHyperStrip.push_back(11);
  mockAngles.push_back(76.2765); mockHyperStrip.push_back(12);
  mockAngles.push_back(76.3333); mockHyperStrip.push_back(13);
  mockAngles.push_back(76.5014); mockHyperStrip.push_back(14);
  mockAngles.push_back(76.2613); mockHyperStrip.push_back(15);
  mockAngles.push_back(75.6751); mockHyperStrip.push_back(16);
  mockAngles.push_back(75.0556); mockHyperStrip.push_back(17);
  mockAngles.push_back(74.7885); mockHyperStrip.push_back(18);
  mockAngles.push_back(74.2397); mockHyperStrip.push_back(19);
  mockAngles.push_back(73.5125); mockHyperStrip.push_back(20);
  mockAngles.push_back(72.7077); mockHyperStrip.push_back(21);
  mockAngles.push_back(72.3269); mockHyperStrip.push_back(22);
  mockAngles.push_back(71.8055); mockHyperStrip.push_back(23);
  mockAngles.push_back(71.2278); mockHyperStrip.push_back(24);
  mockAngles.push_back(70.6297); mockHyperStrip.push_back(25);
  mockAngles.push_back(70.3866); mockHyperStrip.push_back(26);
  mockAngles.push_back(70.1072); mockHyperStrip.push_back(27);
  mockAngles.push_back(69.837);  mockHyperStrip.push_back(28);
  mockAngles.push_back(69.6915); mockHyperStrip.push_back(29);
  mockAngles.push_back(69.7661); mockHyperStrip.push_back(30);
  mockAngles.push_back(69.8655); mockHyperStrip.push_back(31);
  mockAngles.push_back(69.9773); mockHyperStrip.push_back(32);

  TGraphErrors* grStripvsMockData = new TGraphErrors(mockAngles.size(),&mockHyperStrip[0],&mockAngles[0],&gNull[0],&gNull[0]);
  grStripvsMockData->SetMarkerColor(4);
  grStripvsMockData->SetMarkerStyle(3);*/
  ...
  TGraph* grStripvsZData = new TGraphErrors(gAssumedAngle.size(),&gHyperStrip[0],&gHitOnStripZData[0]);
  grStripvsZData->SetTitle(" HitPosition(Z) from data vs HyperStrip");
  grStripvsZData->SetMarkerStyle(20);

  TGraph* grPhivsZ = new TGraphErrors(gAssumedAngle.size(),&gPhiStrip[0],&gHitOnStripZ[0]);
  grPhivsZ->SetTitle(" Phi vs Hit Position (Z) ");
  grPhivsZ->SetMarkerStyle(20);

  TGraph* grPhivsZData = new TGraphErrors(gAssumedAngle.size(),&gPhiStrip[0],&gHitOnStripZData[0]);
  grPhivsZData->SetTitle(" Phi vs Hit Position (Z) from data ");
  grPhivsZData->SetMarkerStyle(20);
  ...
  TCanvas* can3= new TCanvas("can3","can3",650,650);
  can3->Divide(2,1);
  can3->cd(1);
  grStripvsZ->Draw("alp");
  can3->cd(2);
  grStripvsZData->Draw("alp");
  can3->Draw();

  TCanvas* can4= new TCanvas("can4","can4",650,650);
  can4->Divide(2,1);
  can4->cd(1);
  grPhivsZ->Draw("alp");
  can4->cd(2);
  grPhivsZData->Draw("alp");
  can4->Draw();

  TCanvas* can5 = new TCanvas("can5","can5",650,650);
  grPhivsZData->SetMarkerColor(2);
  grPhivsZData->Draw("alp");
  grPhivsZ->Draw("same");
  can5->Draw();
...
double GetChiSquare(const double parameters[]){
  ...
  gHitOnStrip.clear();
  gHitOnStripData.clear();
  gHitOnStripZ.clear();
  gHitOnStripZData.clear();
 ...
 // Construct the absolute positions of the central line along every strip
  for (unsigned i = 0 ; i < gHyperStrip.size() ; i++ ){
    ...
    cout << "\n angle = " << gAngle/deg << " and z = " << gHitOnStripZ[i] << endl;
    //Calculate assumed angle
    double angle = GetParticleAngle(HitPosition,gTargetPos);
    gAssumedAngle.push_back(angle*TMath::RadToDeg());
    angle = GetParticlePhiAngle(HitPosition,gTargetPos);
    gAssumedPhiAngle.push_back(angle*TMath::RadToDeg());

    // perform calculations again but this time with the angle from the input data file
    TVector3 direction2(0,0,1);
    direction2.Rotate(gDataAngle[i]*(TMath::Pi()/180), norm);
    clsLine3D* ActionLine2 = new clsLine3D(BeamSpot, direction2);
    TVector3 HitPosition2 = GetLinePlaneIntersect(ActionLine2,StripPlane);
    gHitOnStripData.push_back(HitPosition2);
    gHitOnStripZData.push_back(HitPosition2.Z());
    cout << " angle = " << gDataAngle[i] << " and z = " << gHitOnStripZData[i] << endl;
  }
