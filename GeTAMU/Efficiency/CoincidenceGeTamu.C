...
void CoincidenceGeTamu(TString Co60FileName="/home/shuyaota/nptool/Outputs/Analysis/EXPT6_ER73_0.root", bool AngularCorrelation=true)
{

	//generate the outputFileName
	TString plotsFileName( Co60FileName( Co60FileName.Last('/')+1, Co60FileName.Length() ) );
...
//////////////////////////////////////////////READ 60Co DATA File and Make Histograms///////////////////////////////////////////////////////////////////
	...
	{ //checks if the file exist already, condition is "true" if not
		...
		//Open TFile
		TFile* nptDataFile = new TFile(Co60FileName.Data(),"OPEN");
	 	if (nptDataFile == 0)
		{
...
//////////////////////////////////////////////FIT 60Co File /////////////////////////////////////////////////////////////////////////////////////////
	...
	//ex., y[0][3] means efficiency of CL4, determined (normalized) by CL1,  y[4][3] means efficiency of CL4, determined by CL1,2,3's average.
	double	x[5][4], ex[5][4], y[5][4], ey[5][4];
	for(int i=0;i<5;i++)
	{
		for(int j=0;j<4;j++)
		{
			x[i][j] = j+1;
			//these are just a trick to make the data plot more visible later.
			if(i==0)	x[i][j]-=0.1;
			else if(i==1)	x[i][j]-=0.05;
			else if(i==2)	x[i][j]+=0.05;
			else if(i==3)	x[i][j]+=0.1;
			else if(i==4)	x[i][j];
			...
			if (currentHist->GetEntries()>100)
			{
				...
				for(unsigned int j=0; j<NumOfPeak; j++)
				{
					...
					if(iClover2==-1)
					{
						...
						//cout << SourceActivity << " " << BGActivityRatio << endl;
					}
					...
					else if(iClover2>=0)
					{
						...
						else if(j==1)
						{
							...
							//these are just a trick to make the data plot more visible later. So don't mind.
							if(iClover==0)	x[iClover][iClover2]-=0.1;
							else if(iClover==1)	x[iClover][iClover2]-=0.05;
							else if(iClover==2)	x[iClover][iClover2]+=0.05;
							else if(iClover==3)	x[iClover][iClover2]+=0.1;
							...
							AngularCor[iClover][iClover2] = CoincidenceHit[iClover][iClover2] / InitialHit[0][iClover];
...
//////////////////////////////////////////////MAKE A GRAPH OF 60Co EFFICIENCY AS A FUNCTION OF CLOVER # ///////////////////////////////////////////////////////////////////
	...
	TCanvas *cD2;
  	TGraphErrors *grCal[5];
	for(unsigned int iClover =0; iClover<=4 ; iClover++)
	{
		//Making an average efficiency of the efficiencies by three clovers.
		for(unsigned int iClover2 =0; iClover2<4 ; iClover2++)
		{
			if(iClover==iClover2)	continue;	//because no efficiency measurements in the same clover (ex., CL1-CL1).

			if(iClover<4)
			{
				y[4][iClover2] += y[iClover][iClover2]; 
				ey[4][iClover2] += pow(ey[iClover][iClover2],2.0); 
			}
			else if(iClover==4)
			{
				y[iClover][iClover2] /= 3.0;
				ey[iClover][iClover2] = pow(ey[iClover][iClover2],0.5)/3.0;
			}
		}

		if(iClover==0)
		{
			sprintf(cName,"Ge_CoincidenceEfficiency");
		...
		sprintf(tgName,"Ge_CoincidenceEfficiency_Clover%d", iClover+1);
		...
		if(iClover==0)
		{
			...
			grCal[iClover]->GetYaxis()->SetRangeUser(0.,0.025);
		//	cD2->Update();
		}
		else if(iClover==1)	
		{
		...
		//Efficiency averaged over other 3 clovers. (ex., for CL1, average CL2-4).
		else if(iClover==4)
		{
                	grCal[iClover]->SetLineColor(1);
                	grCal[iClover]->SetMarkerColor(1);
                	grCal[iClover]->SetMarkerStyle(21);
			grCal[iClover]->Draw("P & same");

			sprintf(imgName,"Ge_CoincidenceEfficiency");
	        	sprintf(ext,"png");
...
//////////////////////////////////////////////WRITE EFFICIENCY TO FILE ///////////////////////////////////////////////////////////////////
	...
	//write in a text file. 
  	TString EfficiencyfName( Co60FileName( Co60FileName.Last('/')+1, Co60FileName.Length() ) );
	...
	for(unsigned int iClover2 = 0 ; iClover2 < 4 ; iClover2++)	
	{
		myfile << iClover2+1<<" " ;
		myfile << Epeaks[0] <<" " ;
		myfile << y[4][iClover2] / BGActivityRatio[iClover2] <<" " ;
		myfile << ey[4][iClover2] / BGActivityRatio[iClover2] <<" " ;
		myfile << Epeaks[1] <<" " ;
		myfile << y[4][iClover2]<<" " ;
		myfile << ey[4][iClover2]<<" " ;
		myfile<<endl;
	}
	myfile.close();

	//if you want to see the all efficiencies determined by three clovers, comment this block out.
	/*
	for(unsigned int iClover = 0 ; iClover < 4 ; iClover++)
	{
		cout << iClover+1<<" " ;
		for(unsigned int iClover2 = 0 ; iClover2 < 4 ; iClover2++)	
		{
			cout << y[iClover][iClover2]<<" " ;
			cout << ey[iClover][iClover2]<<" " ;
		}
		cout<<endl;
	}
	*/
